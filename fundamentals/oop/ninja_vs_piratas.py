...
class Pirata(Brigido):
    multiplicador_ataque = 1.2

    @classmethod
    def pistola(cls, enemigo):
        ataque = random.randint(15, 25) * cls.multiplicador_ataque
        enemigo.recibir_ataque(ataque)
        print(f"{cls.__name__} usó pistola y causó {ataque} de daño.")

    @staticmethod
    def garfio(enemigo):
        ataque = random.randint(25, 35)
        enemigo.recibir_ataque(ataque)
        print(f"{Pirata.__name__} usó garfio y causó {ataque} de daño.")

    def atacar(self, enemigo):
        opciones_ataque = [self.pistola, self.garfio]
        ataque_elegido = random.choice(opciones_ataque)
        ataque_elegido(enemigo)
